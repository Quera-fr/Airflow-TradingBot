...
if st.checkbox("Voir le contenu complet du fichier mémoire", value=True):
    decisions = {}
    for memo in memory.get("recent_decisions", [])[::-1]:
        try: ts = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(memo["ts"]))
        except:ts = None

        try:action = memo["action"]
        except:action = None

        try:time_sleep = int(memo["time_sleep"])
        except:time_sleep = None

        try: reason = memo["reason"].replace("\n", " ")
        except:reason = None

        try:next_steps = memo['next_steps'].replace("\n", " ")
        except:next_steps = None
...
if cap_hist:
    ...
    chart = (
        ...
        .encode(
            ...
            y=alt.Y("c:Q", title="Capital", scale=alt.Scale(domain=[ymin-2, ymax+2])),
